...
# The below document can be inserted into a moodle quiz through the following steps:
  # after creating a moodle quiz question, toggle the toolbar in "question text" and open up the html editor < > button. 
  # knit this document to Html, open in chrome and on the page right click, inspect, and under the html tab, copy the first line with the web address present.
  # paste into the html editor of moodle quiz and click update. 

  # From here, insert moodle questions using the coding syntax which will be written in the markdown file. R might screw with some of the formatting, so insert ~'s when necassary. 

  # once done, preview the question, and set the attempt options as needed. 
```
## Introduction
...
R is a language and environment for statistical computing and graphics. R is free/open source software and as a result, has a community of dedicated statisticans, coders and developers increasing the capabilities and usability of the platform. 
R primarily runs as a command-line program.  

This is a big entry barrier to many starting to learn R, so most people have turned to "R Studio".  

![*Base R*](/images/r_cmd.png?width=50pc)  
...
![*The R Studio Environment*](/images/rstudio.png?width=100pc)  
...
```{R moodle questions, eval=FALSE, include=FALSE}
**Question: Match the following uses for each R studio panel:**

* Console: {1:MULTICHOICE:Top Left #Wrong~=Bottom Left #OK~Top Right #Wrong~Bottom Right #Wrong}

* Workspace: {1:MULTICHOICE:Top Left #Wrong~Bottom Left #Wrong~=Top Right #OK~Bottom Right #Wrong}

* Help/Graph Output: {1:MULTICHOICE:Top Left #Wrong~Bottom Left #Wrong~Top Right #Wrong~=Bottom Right #OK}

* Script Editor: {1:MULTICHOICE:=Top Left #OK~Bottom Left #Wrong~Top Right #Wrong~Bottom Right #Wrong}
